...
새 세션도 버튼으로 시작할 수 있습니다. `/menu -> 새 세션`을 누르면 먼저 리포 선택 화면이 열리고, `기본 리포`, 현재 붙어 있는 세션들이 쓰는 리포, 최근 Codex 세션에서 본 리포가 버튼으로 보입니다. 원하는 리포를 고른 뒤 일반 메시지 한 번을 세션 이름 입력으로 해석합니다. `직접 리포 경로 입력`을 고른 경우에는 `세션명 /absolute/path` 형식으로 보내면 됩니다.
